...
Egg_nog_mapper <- read.csv("D:/Zenodo/Data_R_plots/Fig.5/Egg_nog_mapper.csv")

##Select only Chlorophyta and Viridiplantae
Egg_nog_mapper_1<- Egg_nog_mapper[Egg_nog_mapper$max_annot_lvl %like% "Chlorophyta", ]
Egg_nog_mapper_2<- Egg_nog_mapper[Egg_nog_mapper$max_annot_lvl %like% "Viridiplantae", ]

Egg_nog_mapper<- rbind(Egg_nog_mapper_1, Egg_nog_mapper_2)
...
DFF5<- DFF[DFF$Pathway %like% "00710", ]


#write.csv(DFF5, "D:/Zenodo/Data_R_plots/Fig.5/Photosynthesis_genes_chlorella.csv", row.names = F)
...
#kegg_genesbr6<- kegg_genesbr5[, c("Acc", "query", "count", "Pathway")]
...
#kegg_genesbr7<- kegg_genesbr6 %>%
#  dplyr::group_by(KEGG_Pathway, Acc) %>%
#  dplyr::summarise(across(count, sum))
...
Fig.5D<- ggplot(kegg_genesbr8b, aes(y = Acc, x=as.numeric(percent), fill=Pathway2)) + 
  ...
  theme(axis.text=element_text(size=12),axis.title=element_text(size=12))+
   theme(axis.title.y = element_blank())+
    theme(axis.ticks.y = element_blank())+
    theme(axis.text.y = element_blank())+
  xlab("Relative composition (C,N, S genes)")+
...
lib_17m_mapped_5m_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_5m_r.txt")
lib_17m_mapped_5m_r$Depth <- "5m"


lib_17m_mapped_9m_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_9m_r.txt")
lib_17m_mapped_9m_r$Depth <- "9m"


lib_17m_mapped_11m_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_11m_r.txt")
lib_17m_mapped_11m_r$Depth <- "11m"


lib_17m_mapped_13m_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_13m_r.txt")
lib_17m_mapped_13m_r$Depth <- "13m"


lib_17m_mapped_15mm_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_15mm_r.txt")
lib_17m_mapped_15mm_r$Depth <- "15mm"


lib_17m_mapped_15mw_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_15mw_r.txt")
lib_17m_mapped_15mw_r$Depth <- "15mw"


lib_17m_mapped_15_5m_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_15_5m_r.txt")
lib_17m_mapped_15_5m_r$Depth <- "15_5m"


lib_17m_mapped_17m_r <- read.delim("D:/Zenodo/Data_R_plots/Fig.5/Chlorella_CNS_genes_cov/Cov_mapped_17m_r.txt")
lib_17m_mapped_17m_r$Depth <- "17m"
...
#Fig.5 NEW
((Fig.5A | Fig.5B | Fig.5C) / (Fig.5H )) + plot_layout(heights =  c(1, 1.25), widths = c(1,1.25)) #8x8


((((Fig.5A | Fig.5B | Fig.5C) / (Fig.5H )) + plot_layout(heights =  c(1, 1.25), widths = c(1,1.25))) | 
((Fig.5E  | Fig.5D  | Fig.5F | Fig.5G)))  + plot_layout(heights =  c(1, 1.25), widths = c(1,2))
...
#Drap GPM from Salmon
TPM_salmon_gff3_egg3a= TPM_salmon_gff3_egg3a[ , -which(names(TPM_salmon_gff3_egg3a) %in% c("GPM"))]
...
#HSP1_3<- HSP1_2[HSP1_2$Description3 %like% "HSP70", ]

#reshape(HSP1_3, idvar = "name", timevar = "numbers", direction = "wide")
#unique(HSP1_3$seed_ortholog)
...
HSP1_2$Category = factor(HSP1_2$Category, levels=c('LC_MAG','GCA'))
 ...
 # scale_fill_manual(values=c("lavenderblush", "lavenderblush2", "goldenrod1", "goldenrod2","goldenrod", "goldenrod3", "goldenrod4", "lightskyblue1", "lightskyblue2", 
#                                            "lightskyblue3", "lightskyblue4", "magenta3", "magenta4"))
...
####################################################################################
####################################################################################
Plot_b2_hsp<- ggplot(data=HSP1_2, aes(y=name, x=meandepthcov, color= Description3, shape=max_annot_lvl))+
  ...
  theme_bw()+
  ylab("Genomes")+
  xlab("Mean depth of gene coverage")+
  ...
  xlim(0,60)+
  theme(axis.text.x=element_text(angle=45, hjust=1))+
  scale_shape_manual("EggNOGG annotation", values = c(21,22))+
  theme(legend.position="bottom")+
  geom_point(size=2.5, stroke=2.5)+
  theme(axis.text=element_text(size=12),axis.title=element_text(size=12))
...
(Plot_a_hsp1 | Plot_b2_hsp) + plot_layout(widths = c(0.45, 1.75))#12x8
